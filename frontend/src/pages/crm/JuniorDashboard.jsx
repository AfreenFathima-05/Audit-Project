...
import { AlertCircle, Upload, Check, CheckCircle, ClipboardList, FolderOpen, Flag } from 'lucide-react';
...
const JuniorDashboard = () => {
  const { currentUser, tasks, users, logout, submitReport, startTask } = useCRM();
  ...
  return (
    <div className="min-h-screen bg-portal-bg font-sans pb-20 flex flex-col">
      <PortalTopbar
        ...
        subtitle="Auditor Workspace"
        userName={currentUser.name}
        userRole="Field Auditor"
        onLogout={logout}
      />
      ...
      <div className="flex flex-1">
        ...
        <div className="flex-1 px-6 lg:px-12 py-10 w-full overflow-x-hidden bg-gradient-to-br from-[#F4F6F8] to-[#E9EDF1]">
          ...
          <div>
            ...
            <p className="text-portal-muted">Review your assigned engagements and submit your fieldwork findings.</p>
          </div>
          <div className="flex flex-wrap lg:flex-nowrap gap-4 w-full">
            <StatCard icon={ClipboardList} label="My Tasks" value={myTasks.length} accent="gold" />
            <StatCard icon={AlertCircle} label="Active" value={statusCounts.in_progress} accent="blue" />
            <StatCard icon={CheckCircle} label="Approved" value={statusCounts.completed} accent="success" />
          </div>
        </motion.div>

        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-10">
          <div className="lg:col-span-2 bg-portal-card p-6 rounded-xl border border-portal-line shadow-sm">
            <h3 className="text-lg font-serif text-portal-ink mb-2 flex items-center gap-2"><FolderOpen size={18} className="text-portal-gold" /> Quick Resources</h3>
            <p className="text-sm text-portal-muted mb-5">Reference material you'll likely need during fieldwork.</p>
            <ul className="space-y-3 text-sm">
              {['Audit Working Paper Template', 'Sampling & Materiality Guidelines', 'Internal Controls Checklist'].map((r) => (
                <li key={r} className="flex items-center justify-between px-4 py-3 rounded-lg bg-portal-bg border border-portal-line">
                  <span className="text-portal-ink font-medium">{r}</span>
                  <span className="text-xs text-portal-muted uppercase tracking-widest">PDF</span>
                </li>
              ))}
            </ul>
          </div>
          <div className="bg-portal-card p-6 rounded-xl border border-portal-line shadow-sm">
            <h3 className="text-lg font-serif text-portal-ink mb-4">My Status Mix</h3>
            <DonutChart
              size={140}
              strokeWidth={16}
              segments={[
                { label: 'Pending', value: statusCounts.pending, color: 'var(--color-portal-warning)' },
                { label: 'Active', value: statusCounts.in_progress, color: 'var(--color-portal-gold)' },
                { label: 'Review', value: statusCounts.review, color: 'var(--color-portal-review)' },
                { label: 'Done', value: statusCounts.completed, color: 'var(--color-portal-success)' },
              ]}
            />
          </div>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="space-y-6">
          <h3 className="text-xl font-serif text-portal-ink pl-2 border-l-4 border-theme-olive">Assigned Engagements</h3>
                ...
                <div className="p-6 md:p-8 flex flex-col md:flex-row justify-between gap-6">
                  <div className="space-y-4 flex-grow">
                    ...
                    <div className="bg-portal-bg p-4 rounded-md border border-portal-line text-sm">
                      ...
                      <span className="font-medium text-portal-ink text-base">{client?.name}</span>
                    </div>
                  ...
                  <div className="flex flex-col justify-center bg-portal-bg p-6 rounded-lg border border-portal-line min-w-[280px]">
                    ...
                    {task.status === 'in_progress' && (
                      <div className="space-y-3">
                        ...
                        />
                        <button onClick={() => {
                          if(reportUrl) { submitReport(task.id, reportUrl); setReportUrl(''); }
                    ...
                    {/* Document UI Addition */}
                    {task.status !== 'completed' && (
                      <div className="mt-4 pt-4 border-t border-portal-line">
                        <p className="text-sm font-semibold text-portal-ink mb-2">Required Documentation</p>
                        <div className="flex flex-wrap gap-3">
                          <div className="flex items-center gap-2 border border-portal-line rounded-md px-3 py-1.5 text-xs bg-portal-bg">
                            <span className="w-2 h-2 rounded-full bg-portal-success"></span> Trial Balance
                          </div>
                          <div className="flex items-center gap-2 border border-portal-line rounded-md px-3 py-1.5 text-xs bg-portal-bg">
                            <span className="w-2 h-2 rounded-full bg-portal-warning"></span> Bank Statements
                          </div>
                          <div className="flex items-center gap-2 border border-portal-line rounded-md px-3 py-1.5 text-xs bg-portal-bg">
                            <span className="w-2 h-2 rounded-full bg-portal-danger"></span> Tax Returns (Missing)
                          </div>
                        </div>
                      </div>
                    )}
